...
namespace SplineSpace
{
    ...
    public:
        ...
        bool SinglePointInterp(const double& x,double& y)noexcept(false);		//虚函数实现
        bool MultiPointInterp(const double* x,const int& num,double* y)noexcept(false);
        bool AutoInterp(const int& num,double* x,double* y)noexcept(false);
